table 50053 "Posted Product Design Line"
{
    ...
    fields
    {
        ...
        field(11;"No.";Code[10])
        {
            TableRelation = IF (Type=CONST(" ")) "Standard Text"
                            ELSE IF (Type=CONST("G/L Account")) "G/L Account"."No." WHERE ("Aval. In Estimate"=CONST(true))
                            ELSE IF (Type=CONST(Item),
                                     GSM=FILTER(0),
                                     "Deckle Size"=FILTER(0)) Item."No." WHERE ("Available in Estimate Line"=FILTER(true))
                                     ELSE IF (Type=CONST("Work Center")) "Work Center"."No." WHERE ("Work Center Category"=FIELD("Work Center Category"))
        ...
        field(14;"Unit Of Measure";Code[10])
        {
            TableRelation = IF (Type=CONST(Item)) "Item Unit of Measure".Code WHERE ("Item No."=FIELD("No."))
                            ELSE IF (Type=CONST("Work Center")) "Capacity Unit of Measure"
                            ELSE IF (Type=FILTER("Machine Center")) "Capacity Unit of Measure"
                            ELSE IF (Type=CONST("G/L Account")) "Unit of Measure";
