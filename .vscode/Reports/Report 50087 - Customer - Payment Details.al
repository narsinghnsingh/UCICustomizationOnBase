report 50087 "Customer - Payment Details"
{
    // version Sales/Sadaf

    DefaultLayout = RDLC;
    RDLCLayout = '.vscode/Reports/Customer - Payment Details.rdl';
    ...
    dataset
    {
        dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
        {
            ...
            column(StartBalanceLCY; 'Opening Balance as on ' + Format(StratDate) + '..................... ' + Format(RunningBalance))
            {
            }
            column(StartDate; 'Opening Balance as On' + Format(StratDate))
            {
            }
            dataitem(PageLoop; "Integer")
            {
                ...
                dataitem(Total; "Integer")
                {
                    ...
                    column(CustLedgerEntryDocumentType; "Cust. Ledger Entry"."Document Type")
                    {
                    }
                    column(CustLedgerEntryDocumentNo; "Cust. Ledger Entry"."Document No.")
                    {
                    }
                    column(CustLedgerEntryPostingDate; "Cust. Ledger Entry"."Posting Date")
                    {
                    }
                    column(CustLedgerEntryRemainingAmount; -"Cust. Ledger Entry"."Remaining Amount")
                    {
                    }
                    ...
                    column(DODATE; DODATE)
                    {
                    }
                    column(CHQ_NO; CHQ_NO)
                    {
                    }
                    column(CHQ_DATE; CHQ_DATE)
                    {
                    }
                    ...
                    begin
                        ...
                        BankAccountLedgerEntry.SetRange(BankAccountLedgerEntry."Document No.", "Cust. Ledger Entry"."Document No.");
                        if BankAccountLedgerEntry.FindFirst then begin
                            CHQ_NO := BankAccountLedgerEntry."Cheque No.";
                            CHQ_DATE := BankAccountLedgerEntry."Cheque Date";
                        end;

                        SALES_INVHEADER.Reset;
                        SALES_INVHEADER.SetRange(SALES_INVHEADER."No.", "Cust. Ledger Entry"."Document No.");
                        if SALES_INVHEADER.FindFirst then begin
                            LPONO := SALES_INVHEADER."External Document No.";
                            LPODATE := SALES_INVHEADER."Order Date";
                        end;
                        //MESSAGE(LPONO);

                        SALES_INVLINE.Reset;
                        SALES_INVLINE.SetRange(SALES_INVLINE."Document No.", "Cust. Ledger Entry"."Document No.");
                        if SALES_INVLINE.FindFirst then begin
                            DONO := SALES_INVLINE."Shipment No.";
                            DODATE := SALES_INVLINE."Shipment Date";
                        end;
                        RunningBalance := RunningBalance + "Cust. Ledger Entry"."Original Amount";//"Cust. Ledger Entry"."Amount (LCY)";
            ...
            trigger OnAfterGetRecord()
            begin
                ALLFILTERS := "Cust. Ledger Entry".GetFilters;
                RunningBalance := RunningBalance + "Cust. Ledger Entry"."Original Amount";//"Cust. Ledger Entry"."Amount (LCY)";
                StartBalanceLCY := RunningBalance;
            ...
            begin
                ...
                StratDate := "Cust. Ledger Entry".GetRangeMin("Cust. Ledger Entry"."Posting Date") - 1;
                CustomerNumber := "Cust. Ledger Entry".GetFilter("Cust. Ledger Entry"."Customer No.");
                StartBalanceLCY := 0;
                Cust.Reset;
                Cust.SetRange(Cust."No.", CustomerNumber);
                Cust.SetRange(Cust."Date Filter", 0D, (StratDate));
                if Cust.FindFirst then begin
                    Cust.CalcFields(Cust."Net Change (LCY)");
                    StartBalanceLCY := Cust."Net Change (LCY)";
    ...
    var
        ...
        Text003: Label 'Customer Ledger Details';
        Text004: Label 'Customer Ledger Details';
        ...
        PaymAmtSpecificationCptnLbl: Label 'Amount Specification';
        ...
        PaymAmtNotAllocatedCptnLbl: Label 'Amount Not Allocated';
        CustLedgEntryOrgAmtCptnLbl: Label 'Amount';
        ExternalDocumentNoCaptionLbl: Label 'External Document No.';
        ALLFILTERS: Text;
        SALES_INVHEADER: Record "Sales Invoice Header";
        LPONO: Code[50];
        LPODATE: Date;
        DONO: Code[50];
        DODATE: Date;
        SALES_INVLINE: Record "Sales Invoice Line";
